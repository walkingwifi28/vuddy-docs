...
const learningUrl = new URL(
    "../pages/service-guide/learning-vuddy/index.html",
    import.meta.url,
);
...
async function readLearning() {
    return readFile(learningUrl, "utf8");
}
...
test("learning presentation uses the 15-slide profile structure", async () => {
...
test("learning presentation consolidates three chapters into four before-after cards", async () => {
    const html = await readLearning();
...
test("learning presentation uses the profile CSS and BIZ UDPGothic setup", async () => {
    const [html, profileHtml] = await Promise.all([
        readLearning(),
        readFile(profileUrl, "utf8"),
    ]);
...
test("learning presentation uses an iframe and keeps learning-specific content", async () => {
    const html = await readLearning();

    assert.match(html, /<iframe\b/);
    assert.doesNotMatch(html, /class="learning-demo"/);
    assert.match(html, /learning-vuddy-service-introduction\.pdf/);
